...
class _PointerGroupHandlerState extends State<PointerGroupHandler> {
  ...
  void _setIgnoringStateForAll(bool ignore,
      [_PointersManger? except, bool? valueForExcept = false]) {
  ...
  void _onPointerUpOrCancel({
    ...
    if (firstClickedPointerManger.havePointers) {
      ...
      if (!firstClickedPointerManger.containsPointer(pointerId)) {
        return;
      }
      ...
      if (maxNumOfPointers == null ||
          firstClickedPointerManger.numOfRegisteredPointers <
              maxNumOfPointers) {
